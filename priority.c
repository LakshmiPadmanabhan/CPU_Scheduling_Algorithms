...
void swap(int i,int j,int arrival_time[],int execution_time[],int process_name[],int priority[]){
	int t1,t2,t3,t4;
...
void main()
{
	int t1,t2,t3,t4,n,i,j,min,pos;
	int arrival_time[50];int execution_time[50];int process_name[50];int priority[50];
	...
	for(i=0;i<n;i++)
	{
		for(j=0;j<n-1-i;j++)
		{
			...
			{
				swap(j,j+1,arrival_time,execution_time,process_name,priority);
			}
		}	
	}
